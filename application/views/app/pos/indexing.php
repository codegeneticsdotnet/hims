...
<script language="javascript">
    $(document).ready(function() {
        $('#category').change(function() {
            ...
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url();?>app/billing/getItem/' + category_id,
                success: function(response) {
                    $('#showCategories').html(response);
                }
            });
        });
        $('#medicine_cat').change(function() {
            var category_id = $(this).val();
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url();?>app/billing/drug_list/' + category_id,
                success: function(response) {
                    $('#showDrugListItem').html(response);
                }
            });
        });
       ...
       var engine = new Bloodhound({
            remote: {
                url: "<?php echo base_url();?>app/billing/itemsearch/%QUERY",
                wildcard: '%QUERY'
            },
        ...
        $('#tboxitemname').bind('typeahead:select', function(ev, suggestion) {
            $('#tboxitemname').val(suggestion.label);
            // Add further handling if needed
        });        

    });

    function showDrugNames(category_id){
        //change to ajax
        if (window.XMLHttpRequest){
            xmlhttp=new XMLHttpRequest();
        }else{// code for IE6,
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <tr id="parti" >
                                            ...
                                            <input type="text" name="rate" id="rate" placeholder="rate" class="form-control input-sm" style="width: 250px;" required>
                                                ...
                                                <select name="category" id="category" class="form-control input-sm" style="width: 100%;" required>
                                                ...
                                                <select name="medicine_cat" id="medicine_cat" class="form-control input-sm" style="width: 100%; display: none;" required>
                                        ...
                                        <tr>
                                        	<td>Qty</td>
                                            <td><input type="text" onkeypress="return isNumberKey(event)" name="qty" id="qty" value="1" placeholder="Qty" class="form-control input-sm" style="width: 30%;" required></td>
                                        </tr>
                                        <tr>
                                        	<td>Rate</td>
                                            <td>
                                            <label id="showRate">
                                            <input type="text" onkeypress="return isNumberKey(event)" onkeyup="computeTotal()" name="rate" id="rate" placeholder="rate" class="form-control input-sm" style="width: 70%;" required>
                                            </label>
                                            
                                            <label id="showDrugRate" style="display:none">
                                            <input type="text" onkeypress="return isNumberKey(event)" onkeyup="computeTotal()" name="drugrate" id="drugrate" placeholder="rate" class="form-control input-sm" style="width: 30%;" required>
                                            </label>
                                            </td>
                                        </tr>
                                        <tr>
